...
pub(super) async fn build_scan_plan(
    ...
    // Both terminal shapes carry fileConstantValues for downstream readers. Materialize the full
    // partition schema even without a predicate; the predicate-narrowed stats schema is sufficient
    // for skipping, but is not sufficient to reconstruct partition columns in the data reader.
    let parts = scan.plan_partition_schema();
