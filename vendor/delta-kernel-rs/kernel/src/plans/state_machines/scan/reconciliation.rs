...
fn data_skipping_filter(predicate: Option<&PredicateRef>, shape: &ScanShape) -> Option<Predicate> {
    ...
    let skip_pred =
        DataSkippingPredicateCreator::new(&partition_columns).eval_sql_where(predicate)?;
...
pub(super) fn build_reconciliation(
    ...
    let skipped = match data_skipping_filter(predicate, shape) {
        Some(skip) => {
            // Partition predicates emitted by DataSkippingPredicateCreator are guarded with
            // `OR(NOT is_add, predicate)` so Remove rows survive reconciliation. The live scan
            // evaluator derives `is_add` from `path IS NOT NULL`; declarative plans must
            // materialize the same field before evaluating the rewritten predicate. Keep this
            // conditional so scans without a partition predicate retain their existing shape.
            let retained = if shape.partition_schema.is_some() {
                retained.insert_col_after(
                    [ADD_NAME, "partitionValues_parsed"],
                    StructField::not_null("is_add", DataType::BOOLEAN),
                    Arc::new(col(ADD_PATH).is_not_null().into()),
                )?
            } else {
                retained
            };
            retained.filter(Arc::new(skip))?
        }
        None => retained,
    };
...
pub(crate) fn log_files_to_rows(
    log_root: &Url,
    files: Vec<FileMeta>,
) -> Result<Vec<Vec<Scalar>>, DeltaError> {
    files
        .into_iter()
        .map(|file| {
