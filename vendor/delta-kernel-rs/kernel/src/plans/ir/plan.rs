...
/// ```text
/// ResultPlan {
///     plan: <ListFiles -> ScanJson -> MaxByVersion -> ... -> Project [RefId(7)]>,
///     result: RefId(7),
/// }
/// ```
///
...
impl ResultPlan {
    /// Borrows the [`PlanNode`] producing [`self.result`](ResultPlan::result), or
    /// returns an error if no node in `self.plan` has that `output` (malformed plan).
    pub fn terminal_node(&self) -> crate::DeltaResult<&PlanNode> {
        self.plan
            .nodes
            .iter()
            .find(|n| n.output == self.result)
            .ok_or_else(|| {
                crate::Error::generic(format!(
                    "plan does not produce result RefId {}",
                    self.result.0
                ))
            })
    }

    /// Consumes `self` and returns the sole [`PlanNode`] if this is a single-node plan
    /// whose only node produces [`self.result`](ResultPlan::result). Errors if the plan
    /// has zero or more than one node, or if the only node's `output` doesn't match
    /// `self.result`.
    pub fn into_single_node_plan(self) -> crate::DeltaResult<PlanNode> {
        let ResultPlan { plan, result } = self;
