...
impl Scan {
    ...
    /// Partition schema carried by declarative scan-file rows.
    ///
    ...
    /// in the logical schema, regardless of predicate. The state-info partition schema is gated
    /// on predicate-driven data skipping, so it is `None` for unfiltered partitioned reads. Scan
    /// file rows are also consumed by native data readers, so they always carry the table's full
    /// partition schema.
    #[cfg(feature = "declarative-plans")]
    pub(crate) fn plan_partition_schema(&self) -> Option<SchemaRef> {
        self.snapshot
            .table_configuration()
